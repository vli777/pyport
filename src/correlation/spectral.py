...
def compute_performance_metrics(returns_df: pd.DataFrame, risk_free_rate: float = 0.0) -> pd.Series:
    """
...
def visualize_clusters_tsne(returns_df: pd.DataFrame, cluster_labels: np.ndarray):
    ...
    scatter = plt.scatter(tsne_results[:, 0], tsne_results[:, 1], c=cluster_labels, cmap='tab20', s=50)
...
def filter_correlated_groups_spectral(
    ...
    if n_clusters is None:
        ...
        logger.info(f"n_clusters not provided. Setting n_clusters = {n_clusters} for {n_assets} assets.")
    ...
    for label, ticker_group in clusters.items():
        ...
        logger.info(f"Cluster {label}: {len(ticker_group)} assets; selected {top_candidates}")
    
    if plot:
        visualize_clusters_tsne(returns_df, cluster_labels)
    
    removed_tickers = set(tickers) - set(selected_tickers)
    logger.info(f"Removed {len(removed_tickers)} assets; {len(selected_tickers)} assets remain.")
