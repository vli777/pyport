...
def output(
    ...
    if len(clean_weights) > max_size:
        ...
        sorted_weights = sorted(
            clean_weights.items(), key=lambda kv: kv[1], reverse=False
        )
    ...
    print(
        f"Cumulative return: {round((portfolio_cumulative_returns[-1] - 1) * 100, 2)}%"
    )
    print(f"Portfolio allocation weights (min {minimum_weight:.2f}):")

    sorted_weights = sorted(
        clean_weights.items(), key=lambda kv: kv[1], reverse=config.sort_by_weights
    )
    ...
    all_cumulative_returns = ((portfolio_cumulative_returns) - 1).join(
