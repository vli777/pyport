...
def is_after_4pm_est():
    ...
    return now.hour > 16 or (
        now.hour == 16 and now.minute >= 1
    )  # Check if it's after 4:01 PM EST


def get_non_holiday_weekdays(start_date, end_date, tz=pytz.timezone("US/Eastern")):
    cal = tc.get_calendar("XNYS")
    open_sessions = cal.sessions_in_range(
        pd.Timestamp(start_date, tz=tz), pd.Timestamp(end_date, tz=tz)
    )

    if open_sessions.empty:
        return start_date, start_date

    first_date, last_date = open_sessions[0].date(), open_sessions[
        -1
