...
def normalize_weights(weights, min_weight):
    ...
    negative_weights = {
        k: v for k, v in weights.items() if v < -abs(min_weight)
    }  # Filter negatives based on absolute min_weight

    total_positive = sum(positive_weights.values())
    total_negative = abs(
        sum(negative_weights.values())
    )  # We want the absolute sum of negatives
    ...
    if min_weight >= 0:
        if total_positive > 0:
            ...
            normalized_positive = {
                k: v / total_positive for k, v in positive_weights.items()
            }
        else:
            normalized_positive = (
                positive_weights  # If no positive weights, leave it as is
            )
    ...
    # Case 2: Normalize both positive and negative weights if min_weight includes negatives
        ...
        normalized_negative = (
            {k: v / total_negative for k, v in negative_weights.items()}
            if total_negative > 0
            else negative_weights
        )
        ...
        return {
            k: v for k, v in normalized_combined.items() if abs(v) > abs(min_weight)
        }
    ...
    for portfolio in stack_dict.values():
        if isinstance(portfolio, dict):
            for symbol in all_symbols:
                total_weights[symbol] += portfolio.get(
                    symbol, 0
                )  # Default to 0 if the symbol is missing

    # Calculate the arithmetic mean by dividing each symbol's total weight by the number of portfolios
    average_holdings = {
        symbol: total_weight / num_portfolios
        for symbol, total_weight in total_weights.items()
    }
