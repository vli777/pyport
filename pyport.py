...
def stacked_output(stack_dict):
    ...
    for model in stack_dict:
        ...
        stack_dict[model] = custom_scaling(weights_dict=portfolio, scaling=scaling_factor / maxlen)
...
def output(
    ...
    if len(scaled) > 0:
        ...
        drawdown, time_under_water = drawdown_and_time_under_water(cumulative_returns)
        ...
        print('portfolio allocation weights (min {:.2f}):'.format(minimum_weight))
    else:
